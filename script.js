...
//   device-tablet  -> reserved for a future distinct tablet layout
//   device-desktop -> rearranged layout (right-side vertical bar,
//                     board that grows with the screen, etc.)
// styles.css reacts to these classes (see "html.device-desktop ..."
// rules), which is what lets the layout fully re-order elements
// instead of just shrinking/growing them. This runs immediately
// (not inside DOMContentLoaded) so the correct class is set before
// the game screen is ever shown, and again on every resize/rotation
// so it keeps up if the window is resized or the device is rotated.
// ============================================================
(function () {
    ...
    function applyDeviceClass() {
        ...
        var next = getDeviceClass();
        DEVICE_CLASSES.forEach(function (c) {
            if (c !== next) html.classList.remove(c);
        });
        if (!html.classList.contains(next)) html.classList.add(next);
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Skipped entirely if there's a valid saved in-progress game — in that case
    // we resume straight into the game page instead (see the end of this file),
    // and this timer must never be allowed to yank the player back to the
    // theme page a few seconds later.
    try {
        const _pages = document.querySelectorAll('.page');
        const _dots = document.querySelectorAll('.dot');
        const _hasSavedGame = (function () {
            try {
                const raw = localStorage.getItem('chessPiGameState');
                if (!raw) return false;
                const parsed = JSON.parse(raw);
                return !!(parsed && typeof parsed.pgn === 'string' && parsed.difficulty);
            } catch (e) {
                return false;
            }
        })();
    ...
    let hiddenAtTimestamp = null; // set when the screen/tab goes to sleep or background
    ...
    function endGame(message, isWin = false) {
        // A finished game should never be auto-resumed on next load.
        clearSavedGameState();
    ...
    function initNewGame() {
        ...
        // A brand-new game is now the current "resumable" snapshot.
        saveGameState();
    }

    // ===================================================================
    // AUTO-SAVE / AUTO-RESUME: persists the live game (board, whose turn,
    // time left, remaining hints/undos/threats/extra-time, move history)
    // so that if the phone screen turns off, the browser is closed, or the
    // page is reloaded, the player is dropped straight back into their
    // game instead of the welcome screen.
    // ===================================================================
    const GAME_STATE_KEY = 'chessPiGameState';

    function saveGameState() {
        // Only meaningful while an actual game is on screen; avoid
        // overwriting a good saved game with junk from the menu pages.
        ...
        try {
            ...
            const state = {
                ...
                hints: userSettings.hints,
                undos: userSettings.undos,
                threats: userSettings.threats,
                extraTime: userSettings.extraTime,
    ...
    function clearSavedGameState() {
        try {
            ...
            console.error('clearSavedGameState failed:', e);
        }
    }
    ...
    function loadSavedGameState() {
        try {
            ...
            if (!raw) return null;
            const parsed = JSON.parse(raw);
            if (!parsed || typeof parsed.pgn !== 'string' || !parsed.difficulty) return null;
            return parsed;
        } catch (e) {
            console.error('loadSavedGameState failed:', e);
            return null;
        }
    }

    // Belt-and-suspenders persistence: moves/feature-use already trigger an
    // immediate save, but the player's clock keeps ticking between moves too
    // (their own thinking time). These make sure that time isn't lost if the
    // phone screen turns off, the tab is backgrounded, or the app is closed
    // mid-thought rather than right after a move.
    setInterval(saveGameState, 5000);

    // IMPORTANT for phone sleep (screen lock), not just closing the page:
    // while the screen is off, mobile browsers throttle or fully pause
    // gameTimer's setInterval, so it can't be trusted to have ticked
    // correctly. We record the exact moment the screen goes dark, and on
    // waking, correct playerTime using real wall-clock elapsed time instead
    // — then restart the interval clean so it stays accurate going forward.
    document.addEventListener('visibilitychange', function () {
        if (document.hidden) {
            hiddenAtTimestamp = Date.now();
            saveGameState();
        } else if (hiddenAtTimestamp) {
            const elapsedSeconds = Math.floor((Date.now() - hiddenAtTimestamp) / 1000);
            hiddenAtTimestamp = null;

            if (currentPage === 3 && userSettings.difficulty !== 'easy' &&
                currentPlayer === 'white' && !isTimerPaused && !game.game_over() && elapsedSeconds > 0) {
                playerTime = Math.max(0, playerTime - elapsedSeconds);
                updateTimerDisplay();
                if (playerTime <= 0) {
                    endGame("Time's up! Black wins by timeout!", false);
                } else {
                    startTimer(); // clean interval synced to right now
                }
            }
            saveGameState();
        }
    ...
    // Rehydrates every piece of saved state and jumps straight to the game
    // page — used once, on page load, instead of the normal "pick a
    // difficulty" flow.
    function restoreGameFromState(state) {
        try {
            game = new Chess();
            if (state.pgn) {
                const loaded = game.load_pgn(state.pgn);
                if (!loaded) game = new Chess();
            }
            moveHistory = Array.isArray(state.moveHistory) ? state.moveHistory : [];

            userSettings.difficulty = ['easy', 'medium', 'hard', 'expert'].includes(state.difficulty)
                ? state.difficulty : 'easy';
            userSettings.theme = ['brown', 'green', 'pink', 'blue'].includes(state.theme)
                ? state.theme : 'brown';
            userSettings.hints = Number.isFinite(state.hints) ? state.hints : userSettings.hints;
            userSettings.undos = Number.isFinite(state.undos) ? state.undos : userSettings.undos;
            userSettings.threats = Number.isFinite(state.threats) ? state.threats : userSettings.threats;
            userSettings.extraTime = Number.isFinite(state.extraTime) ? state.extraTime : userSettings.extraTime;

            currentPlayer = state.currentPlayer === 'black' ? 'black' : 'white';
            playerTime = Number.isFinite(state.playerTime) ? state.playerTime : 0;
            initialTime = Number.isFinite(state.initialTime) ? state.initialTime : 0;
            timeIncrement = Number.isFinite(state.timeIncrement) ? state.timeIncrement : 0;
            gameStats = (state.gameStats && typeof state.gameStats === 'object') ? state.gameStats : gameStats;
            ...
            updatePlayerIndicator();
            updateFeatureButtonsState();
            if (typeof checkRefillButtonState === 'function') checkRefillButtonState();

            const gameOverModal = document.getElementById('game-over-modal');
            if (gameOverModal) gameOverModal.style.display = 'none';

            switchPage(3, true); // resume — do NOT let switchPage call initNewGame()

            // Only restart the clock if it's the player's turn; if it's the
            // bot's turn, its move is triggered below instead.
            if (userSettings.difficulty !== 'easy' && currentPlayer === 'white' && playerTime > 0 && !game.game_over()) {
                startTimer();
            }

            if (currentPlayer === 'black' && !game.game_over()) {
                setTimeout(() => {
                    if (!game.game_over() && game.turn() === 'b' && typeof makeAIMove === 'function') {
                        makeAIMove();
                    }
                }, 400);
            }
            ...
            console.error('restoreGameFromState failed, saved game discarded:', e);
            clearSavedGameState();
        }
    }
    ...
    function switchPage(pageIndex, skipInit) {
        ...
        if (selectedSquare) {
            ...
            if (selectedPiece && selectedPiece.type === 'p' && selectedPiece.color === 'w' && squareName[1] === '8') {
                ...
                if (move) {
                    ...
                    // Persist the game right after every move (board, turn, time).
                    saveGameState();
    ...
    // ===================================================================
    // AUTO-RESUME: if a game was left in progress (phone screen turned
    // off, browser/app reloaded, etc.), jump straight back into it
    // instead of showing the welcome/theme/difficulty flow.
    // ===================================================================
