            ...
            if (toSquareEl) {
                ...
                if (endLeft !== slideInfo.startLeft || endTop !== slideInfo.startTop) {
                    ...
                    requestAnimationFrame(() => {
                        ghost.style.transition = 'left 0.18s cubic-bezier(0.22, 0.61, 0.36, 1), top 0.18s cubic-bezier(0.22, 0.61, 0.36, 1)';
                        ghost.style.left = endLeft + 'px';
                        ghost.style.top = endTop + 'px';
                    });
                    ghost.addEventListener('transitionend', cleanup);
                    setTimeout(cleanup, 260); // safety fallback if transitionend doesn't fire
                }
            }
        }
    }
